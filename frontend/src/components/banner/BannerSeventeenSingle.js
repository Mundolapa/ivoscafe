...
const BannerSixteenSingle = ({ data, spaceBottomClass, textAlign }) => {
    const currency = useSelector((state) => state.currency);
    const price = (data.price * currency.currencyRate).toFixed(2);
  return (
      <div className={clsx("single-banner banner-green-color", spaceBottomClass, textAlign === "right" && "align_right")}>
